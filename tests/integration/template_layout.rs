...
fn test_complete_template_to_layout_flow() {
    ...
    println!("✓ 布局计算完成");
    println!("  画布尺寸: {}x{} dots", layout_result.canvas_width, layout_result.canvas_height);

    if let Some(border) = &layout_result.border {
        println!("  边框: {}x{} at ({}, {}), 线宽 {} dots",
    ...
    println!("  [标题] {}pt at ({}, {}), {}x{} dots",
        title.font_size.unwrap(), title.x, title.y, title.width, title.height);

    // 检查呼号元素
    let callsign = &layout_result.elements[2];
    assert_eq!(callsign.id, "callsign");
    assert_eq!(callsign.content, "BG7XXX");
    assert!(callsign.font_size.unwrap() > title.font_size.unwrap(),
        "呼号字号应该大于标题字号");
    println!("  [呼号] {}pt at ({}, {}), {}x{} dots",
        callsign.font_size.unwrap(), callsign.x, callsign.y, callsign.width, callsign.height);

    // 检查条形码元素
    let barcode = &layout_result.elements[3];
    assert_eq!(barcode.id, "barcode");
    assert_eq!(barcode.content, "BG7XXX");
    assert!(barcode.barcode_config.is_some(), "条形码元素应该有条形码配置");
    println!("  [条形码] at ({}, {}), {}x{} dots",
    ...
    println!("  内容占用高度: {} dots (第一个y={}, 最后一个bottom={})",
    ...
    for element in &layout_result.elements {
        ...
        assert!(offset_from_center < 50,
            "元素 {} 应该水平居中 (偏移: {})", element.id, offset_from_center);
    }
...
fn test_overflow_protection() {
    ...
    let mut data = HashMap::new();
    data.insert("task_name".to_string(), "非常非常长的测试任务名称用于测试溢出保护".to_string());
    data.insert("callsign".to_string(), "BG7XXX".to_string());
    data.insert("sn".to_string(), "001".to_string());
    data.insert("qty".to_string(), "100".to_string());
    ...
    assert!(content_height <= available_height + 50,
        "溢出保护应该确保内容不超出可用区域");
...
fn test_element_spacing() {
    ...
    println!("  配置的行间距: {}mm = {} dots", config.layout.line_gap_mm, line_gap_dots);
    ...
    for i in 0..layout_result.elements.len() - 1 {
        ...
        println!("  元素 {} -> {}: 间距 {} dots (期望 {})",
            current.id, next.id, actual_gap, line_gap_dots);

        // 允许一定误差（由于居中和缩放可能导致小的偏差）
        assert!((actual_gap as i32 - line_gap_dots as i32).abs() <= 5,
            "元素间距应该接近配置值");
    }
