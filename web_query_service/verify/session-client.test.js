...
test('requestQuery: 403（_ts 过期）→ 同会话重签重试一次（不重握手）', async () => {
  let n = 0;
  const deps = mkDeps({ doFetch: async () => { n++; return n === 1 ? { status: 403, json: async () => ({ success: false, message: '签名已过期' }) } : { status: 200, json: async () => ({ ok: true }) }; } });
  const mgr = createSessionManager(deps);
  const res = await mgr.requestQuery('/api/query', {});
  assert.equal(res.status, 200);
  assert.equal(res.retried, true);
  assert.equal(deps.handshakes(), 1, '403 应同会话重签、不重握手（仅 1 次握手）');
});
