    ...
    <el-card shadow="hover">
      <div style="margin-bottom: 20px">
        <p style="color: #909399; font-size: 14px; margin: 0">
          配置 QRZ.com 登录凭据，用于查询呼号地址信息。凭据将加密保存到系统钥匙串或本地加密文件。
        </p>
      </div>
...
const loadCredentials = async (): Promise<void> => {
  try {
    ...
    // 检查登录状态
    isLoggedIn.value = await invoke<boolean>('qrz_com_check_login_status')
  } catch (error) {
...
const handleClearCredentials = async (): Promise<void> => {
  try {
    await ElMessageBox.confirm(
        '确定要清除已保存的 QRZ.com 凭据吗？清除后需要重新登录。',
        '确认清除',
        {
    ...
    await invoke('qrz_com_clear_credentials')
