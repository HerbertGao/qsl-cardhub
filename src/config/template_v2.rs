...
impl TemplateV2Config {
    /// 生成默认的QSL Card v2模板配置
    ///
    /// 符合 docs/template.v2.md 规范的完整模板配置
    pub fn default_qsl_card_v2() -> Self {
        Self {
...
mod tests {
    ...
    #[test]
    fn test_template_v2_config_serialization() {
        let config = TemplateV2Config {
            metadata: TemplateMetadata {
                template_version: "2.0".to_string(),
        ...
        // 反序列化
        let deserialized: TemplateV2Config = toml::from_str(&toml_str).unwrap();
    ...
    fn test_element_validation() {
        let mut config = TemplateV2Config {
            metadata: TemplateMetadata {
                template_version: "2.0".to_string(),
                name: "Test".to_string(),
                version: "1.0".to_string(),
    ...
    fn test_load_and_save() {
        ...
        // 加载配置
        let loaded_config = TemplateV2Config::load_from_file(&temp_file).unwrap();
