...
pub async fn get_profiles(state: State<'_, ProfileState>) -> Result<Vec<Profile>, String> {
    ...
    manager.get_all().map_err(|e| format!("获取配置列表失败: {}", e))
}

/// 根据 ID 获取 Profile
#[tauri::command]
pub async fn get_profile(id: String, state: State<'_, ProfileState>) -> Result<Option<Profile>, String> {
    let manager = state.manager.lock().map_err(|e| format!("锁定失败: {}", e))?;
    manager.get_by_id(&id).map_err(|e| format!("获取配置失败: {}", e))
}
...
pub async fn create_profile(
    ...
    let mut manager = state.manager.lock().map_err(|e| format!("锁定失败: {}", e))?;
    manager
...
/// 获取默认 Profile ID
#[tauri::command]
pub async fn get_default_profile_id(state: State<'_, ProfileState>) -> Result<Option<String>, String> {
    let manager = state.manager.lock().map_err(|e| format!("锁定失败: {}", e))?;
...
/// 从 JSON 导入 Profile
#[tauri::command]
pub async fn import_profile(json: String, state: State<'_, ProfileState>) -> Result<Profile, String> {
    let mut manager = state.manager.lock().map_err(|e| format!("锁定失败: {}", e))?;
    manager
        .import_profile(&json)
