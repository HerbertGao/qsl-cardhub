# 规范：QRZ.herbertgao.me 呼号查询集成 (Phase 2)
...
- **那么** 系统必须在分发弹框的地址展示区域按以下格式显示:
  ...
  ```
- **并且** 各字段显示规则:
  - 呼号: 显示从 API 查询到的呼号，后面跟数据来源标识 `(qrz.herbertgao.me)`
  - 姓名: 用户姓名（来自 `name` 字段）
...
- **并且** 缓存格式为:
  ...
  ```
- **并且** `source` 字段必须固定为 `"qrz.herbertgao.me"`,标识数据来源
...
- **当** 追加新记录后，同一 `source`（qrz.herbertgao.me）的记录超过 2 条
- **那么** 系统必须删除最旧的记录
- **并且** 仅保留最新的 2 条记录
...
- **并且** 所有成功查询的结果都保存到 address_history
- **并且** 按数据来源分组显示
...
- **qrz-com-integration**(Phase 2): QRZ.com 集成(相似功能,不同数据源)

## 技术约束

- **查询接口**: `https://qrz.herbertgao.me/qrz/callsign/<CALLSIGN>`
- **响应格式**: JSON 数组
- **编码格式**: UTF-8
- **认证方式**: 无需认证，公开 API
- **响应字段**:
  ...
  - `isShow`: 是否显示（布尔值）
- **数据筛选**: 仅使用第一个 `isShow: true` 的记录
- **查询模式**: 自动查询（弹框打开时），支持手动刷新
- **缓存有效期**: 365 天（1年，超过显示过期提示）
- **数据来源标识**: 所有查询结果必须标记 `source: "qrz.herbertgao.me"`
...
- **展示格式**:
  ...
  ```
- **错误处理**: 查询失败静默处理，仅记录日志，不显示用户提示
- **并行查询**: 与其他数据源（QRZ.cn、QRZ.com）并行查询，互不影响
- **配置要求**: 无需用户配置，无需登录，自动启用
...
- ✅ 依赖 Phase 2 卡片管理功能
- ✅ 无需配置界面，不影响现有页面布局
- ✅ 与 QRZ.cn 和 QRZ.com 集成并行工作，互不影响
- ✅ 地址历史格式兼容多数据源（qrz.cn、qrz.com、qrz.herbertgao.me）
- ✅ 查询失败不影响用户体验（静默处理）
