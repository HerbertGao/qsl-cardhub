...
- [x] 2.4 验证（前提：`RATE_LIMIT` KV 已绑定，否则 `checkRateLimit` fail-open 不限流）：Worker 构建无未定义引用；同 IP 超额打 auth-callback 被限流，正常单次订阅不受影响；确认计数键源为 `CF-Connecting-IP`、伪造 `X-Forwarded-For` 不改变限流桶。**注（归档后修正认知）**：生产经阿里云 CDN（qsl.herbert-dev.cn）入口时 `CF-Connecting-IP` 实为 CDN 回源 IP、非真实用户，限流粒度失真；真实 IP 取用与来源防伪待阶段 3 修（见 `docs/multi-tenant-design.md` 防爬一节）
...
- [x] 3.4 验证：构造内部异常与微信失败时，响应体均不含原始异常/上游原始结构

## 4. 部署与回归

- [x] 4.1 部署：确认 `CLIENT_SIGN_KEY` 以 Secret 管理 → 部署 Worker
- [x] 4.2 回归：桌面端 `/sync`、移动端查询、微信订阅流程行为不变
