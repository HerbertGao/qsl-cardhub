...
**移除原因**：不再支持云数据库模式，仅使用本地 SQLite。

### ~~需求：云数据库连接配置~~

**移除原因**：不再需要配置 PostgreSQL/MySQL 连接。

### ~~需求：用户认证（注册/登录/JWT）~~

**移除原因**：云端同步使用 API Key 认证，不需要完整的用户系统。

### ~~需求：Repository 抽象层~~

**移除原因**：仅使用 SQLite，不需要数据库抽象层。
...
- **local-credential-storage**（Phase 2）：本地凭据加密存储（用于安全存储 API Key）
- **sf-express-integration**：顺丰速运集成（导出/导入包含顺丰相关数据）

## 技术约束

- **导出格式**：JSON（UTF-8 编码）
- **文件扩展名**：`.qslhub`
- **版本检查**：高版本拒绝导入，低版本或相同版本允许导入
- **导入模式**：覆盖模式（清空后导入），使用事务保证原子性
- **云端认证**：API Key（Bearer Token）
- **同步模式**：全量同步（不支持增量）
- **同步方向**：单向（本地 → 云端）
