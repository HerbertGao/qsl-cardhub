...
按呼号查询（`/api/query`、`/api/callsigns/:callsign`）与顺丰 route-push 的呼号反查 join **必须**按服务端确定的 `tenant_id` 过滤，`tenant_id` **禁止**取自前端参数。本期路由解析尚未引入，读取侧 `tenant_id` **必须**恒为 `default`（host/path → 租户路由属后续阶段）。

#### 场景：查询注入租户过滤

- **当** 调用方按呼号查询
- **那么** 服务端**必须**在 SQL 中注入 `WHERE tenant_id = ? AND callsign = ?`，其中 `tenant_id` 来自服务端上下文（本期为 `default`）
...
- **那么** 查询**必须**由服务端注入确定的 `tenant_id`（本期常量 `default`，即 `WHERE o.tenant_id = ? AND o.tenant_id = c.tenant_id`），而非仅写 `o.tenant_id = c.tenant_id` 的「同租户自洽」（后者在多租户下等于跨租户取第一条匹配）
- **并且** **必须**保留现状业务连接键 `o.card_id = c.id`（隔离键 `client_id` → `tenant_id` 替换不得连带丢失 `card_id` 连接，否则退化为笛卡尔积错号）
- **并且** spec **必须**注明：route-push 是无凭据公开端点、无租户上下文，按 order 派生确定租户的完整方案属阶段 4（见 design 蓝本 §6）；本期恒 `default` 故自洽

#### 场景：openid 反查的租户不变量（本期记录、阶段 4 落地）

- **当** route-push 反查到 callsign 后查 `callsign_openid_bindings`（`index.js:575` `WHERE callsign=?`）选 openid 发推送
- **那么** 隔离契约**必须**声明：该 `callsign → openid` 反查在多租户下**必须**带 tenant 维度，否则两租户同呼号订阅会跨租户推送（openid + 物流轨迹泄漏）
- **并且** `callsign_openid_bindings` 与 `sf_route_log` 本期**不加** `tenant_id`（callsign 为全局键），此为**已知二次迁表点**：spec **必须**显式声明其租户化推迟到阶段 4，且**阶段 4 上线第二个真实租户前必须先迁这两表**（本期「一次到位钉死」仅覆盖业务 5 表 + `sync_meta`，此两全局订阅/日志表显式延后）
