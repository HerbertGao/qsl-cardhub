...
## Purpose
TBD - created by archiving change auto-database-migration. Update Purpose after archive.
## 需求
### 需求：迁移文件自动发现
...
**给定** 在 `migrations/` 目录下添加了新文件 `2026.1.24.001_add_feature.sql`
**当** 应用程序启动时
**则** 系统应该自动识别该迁移文件
**并且** 如果数据库版本低于对应版本号，应该执行该迁移

#### 场景：迁移文件命名规范

**给定** 迁移文件名为 `{YYYY}.{M}.{D}.{NNN}_{description}.sql` 格式
**当** 系统解析文件名时
**则** 应该正确提取版本号并转换为整数
**并且** 忽略不符合格式的文件
