...
#### 场景：连接测试 GET /ping（走 resolveTenant + 回显租户 + 交叉校验）

- **当** 客户端发送 GET 请求到 `{api_url}/ping`，请求头携带 `Authorization: Bearer {api_key}`，并**可选**携带 `X-Tenant-Id: {声明租户 slug}`
- **那么** 服务端鉴权**必须**走 `resolveTenant`（表驱动为主 + `env.API_KEY` 直比兜底，见 `tenant-isolation`），**取代**原「仅直比 `env.API_KEY`」机制——使「多 Key → 同租户」的表驱动凭据也能在 `/ping` 测通（修复阶段 1 凭据表落地后「仅 `env.API_KEY` 那把能测通、表驱动 Key 一律 401」的缺口）
- **并且** `/ping` 的 `resolveTenant` 调用**必须只读**：经 `env.API_KEY` 兜底命中时**禁止**递增 `service_counters('auth_fallback')`（否则探活/测试连接会污染「撤兜底量化判据 `auth_fallback count===0`」）；`/sync`、`/pull` 的兜底命中**仍**计数
- **并且** `trim` 语义保持：`env.API_KEY` 含尾随空白时仍测通（`resolveTenant` 兜底对 `env.API_KEY` 已 `trim`）；`env.API_KEY` 未配置（空/undefined）**且**无表驱动命中时**必须**返回 401，**禁止**沿用「`env.API_KEY` 为空即跳过校验放行」的 fail-open
- **并且** 鉴权通过时返回 200 与 JSON：`{ "success": true, "message": "pong", "server_time": "...", "tenant": "<resolveTenant 解析出的 tenant_id>", "fallback": <布尔> }`——`tenant` 与 `fallback` 两字段**必须**返回（`tenant` 供桌面端「测试连接」确认归属；`fallback` 标记本次是否经 `env.API_KEY` 兜底命中，供 4-C2 模式徽章对接）；`message`、`server_time` 沿用既有（与 `specs/cloud-database-support` 一致）
- **并且** 携带 `X-Tenant-Id` 且与解析租户不一致时**必须**返回 403（见「写入/探活端点声明租户交叉校验」需求）
- **并且** 鉴权失败时返回 401（错误码 `auth_failed`，见同上交叉校验需求的错误码契约）
...
- **并且** 上述「删除该租户全量 + 重新写入 + `sync_meta` 版本写」**必须**置于单个 `DB.batch` 事务，中途失败时**禁止**留下已删未写的空表状态；护栏路径下递增版本的 CAS **必须**为 batch 末条、且版本守卫覆盖全部 `DELETE/INSERT`（见 `cloud-sync-versioning` 原子性约束）；当兜底默认租户（`env.API_KEY` 兜底解析出的 `DEFAULT_TENANT` 指定租户，本部署 `bh2ro`）行数在 D1 单次调用查询上限（Paid 1000 / Free 50，按 batch 内语句条数计、与单语句行数无关）内时为真单 batch 原子（本期数据量预期可容纳）
...
- **并且** 在写入面（同步由 Key 解析、无路径路由；`env.API_KEY` 兜底解析为 `DEFAULT_TENANT` 指定的默认租户，本部署 `bh2ro`）下，等价于仅替换该默认租户数据（写入面租户身份恒由 Key 解析，路径前缀路由仅作用于公共查询面，见 `tenant-path-routing`）
...
### 需求：写入/探活端点声明租户交叉校验（X-Tenant-Id）

`/sync`、`/pull`、`/ping` **必须**接受客户端请求头 `X-Tenant-Id`（声明的租户 slug），并与「写入 Key 经 `resolveTenant` 解析出的 `tenant_id`」做**交叉校验**：不一致**必须**返回 **403**。该机制挡住「误用别租户的 Key → 全量覆盖抹掉对方数据」一类不可逆事故。

**红线**：`X-Tenant-Id` 是客户端可控字段，**仅**用于校验+回显，**禁止**当写入/读取目标——入库/读取的 `tenant_id` **恒**取 `resolveTenant` 的返回值（见 `tenant-isolation`「声明租户交叉校验不变量」）。

**结构化错误码**（客户端区分依据）：Key 无效（`resolveTenant` 不命中）→ **401** `{ "success": false, "code": "auth_failed", "message": "<人类可读>" }`；声明 ≠ 解析 → **403** `{ "success": false, "code": "tenant_mismatch", "message": "<人类可读>" }`。`code` 与 HTTP 状态码是**契约**，`message` **非契约**（人类可读提示，客户端**禁止**依赖其文案）；401 文案沿用各端既有（`/ping` 「API Key 无效」、`/sync`·`/pull` 「认证失败，请检查 API Key」），403 为新响应、文案不约束。`code` 为**向后兼容超集**——既有响应无 `code` 字段、既有客户端按 HTTP 状态码短路不读 `code`，故加字段无害。`CORS_HEADERS` 的 `Access-Control-Allow-Headers` **必须**加入 `X-Tenant-Id`（供浏览器侧 DIY 实现）。

#### 场景：声明与解析一致则放行

- **当** 客户端带 `Authorization: Bearer {key}` + `X-Tenant-Id: {slug}` 请求 `/sync`、`/pull` 或 `/ping`，且 `resolveTenant(key)` 命中的 `tenant_id` 与 `{slug}` 相等
- **那么** 服务端**必须**正常处理（`/sync` 200 全量替换、`/pull` 200 快照、`/ping` 200 回显），用 `resolveTenant` 解析出的 `tenant_id` 入库/读取，与无声明时行为一致

#### 场景：声明与解析不一致则 403 且零改动

- **当** `X-Tenant-Id` 非空且与 `resolveTenant(key)` 解析出的 `tenant_id` 不相等（例如持租户 A 的有效 Key 却声明租户 B）
- **那么** 服务端**必须**返回 403 `{ "success": false, "code": "tenant_mismatch" }`，`/sync` **必须**零数据改动（不进入 DELETE/INSERT）、`/pull` **必须**在任何 SELECT 之前返回（不读取任何租户数据）
- **并且** **禁止**据声明值写入/读取任何租户的数据（绝不写入 B、也绝不退回写入/读取 A 后才校验）——交叉校验**必须**在任何数据改动/读取之前

#### 场景：缺/空 X-Tenant-Id 向后兼容放行

- **当** 客户端**不携带** `X-Tenant-Id`，或携带但为**空串/纯空白**（值经 `trim` 后为空）
- **那么** 服务端**必须**跳过交叉校验、按 `resolveTenant(key)` 正常处理，行为与本变更前等价（归属仍由 Key 决定）——即声明值判空**必须**在 `trim` 之后，空白头等同缺头
- **并且** **禁止**因缺/空声明头而拒绝（否则灰度期旧客户端全挂）；缺头放行不降级安全（归属仍由 Key）
