# query-antibot-session 规范

## 目的

面向**不登录、公开**的「按呼号查询」读取面提供防爬能力：以 PoW（hashcash）门票 + 短时会话（绑真实 IP+UA、HMAC token）+ 单会话查询配额 + 会话专属动态签名密钥 + 按真实 IP（`client_binding_key`，IPv6 /64、IPv4 /32）自适应难度，把「全量爬库成本」抬到不划算（爬全库成本 ≈ 会话数 × PoW）。诚实声明**物理上限**：只抬高成本、**不**杜绝全量遍历；本能力产物（会话/IP 计数）仅用于抬成本，**禁止**作为访问控制/鉴权的唯一闸门。`RATE_LIMIT` KV 为硬部署前置（会话/PoW/配额依赖），KV 未绑或运行时失败时会话相关端点 fail-closed。取代原静态 `CLIENT_SIGN_KEY` 签名与算术验证码。
